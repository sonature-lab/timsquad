...
### 작업 흐름
사용자가 작업을 지시하면 바로 응답한다. 단, 작업 품질을 위해 아래 순서로 점검한다:

1. **개발환경 점검**: 테스트 프레임워크 미설치 시 → "테스트 환경이 없습니다. 설치할까요?"
2. **관련 스펙 점검**: 작업에 필요한 SSOT 문서(\`prd.md\`, Sub-PRD 등)의 충실도 확인
   - 충분 → 그대로 진행
   - 부족(빈 섹션, TBD, placeholder) → "이 작업의 기획 문서가 충분하지 않아 보강이 필요합니다. \`/tsq-grill\`로 보강할까요?"
   - 미존재 → "프로젝트 기획서가 아직 없습니다. \`/tsq-grill prd\`로 만들어볼까요?"
3. **작업 라우팅**: 사용자 확인 후 진행
...
사용자가 스펙 보강을 거절하면 그대로 진행한다. 강제하지 않되, 항상 안내한다.

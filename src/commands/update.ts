...
import { getTemplatesDir, createTemplateVariables, copyAndProcessFile, copyDirectoryWithSubstitution, injectTimsquadBlock } from '../lib/template.js';
...
async function getSyncTargets(_projectRoot: string, config: TimsquadConfig): Promise<string[]> {
  ...
  targets.push('CLAUDE.md (TimSquad block injection)');
...
async function executeUpgrade(
  ...
  // Step 7: CLAUDE.md (inject/update TimSquad block, preserve user content)
...
async function runRollback(options: { yes?: boolean }): Promise<void> {
  ...
  if (!manifest) {
    ...
    console.log(colors.dim('\n  Backups are created automatically by "tsq update".'));
    console.log(colors.dim('  Run "tsq update" first, then "tsq update --rollback" to revert.\n'));
    return;
  }
...
function printUpgradeReport(report: UpgradeReport): void {
  ...
  console.log('');
  console.log(colors.dim('  To revert: tsq update --rollback'));
  console.log('');
}

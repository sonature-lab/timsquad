...
- **스킬**: 37개 `tsq-*` flat namespace
  ...
  - 슬래시: tsq-start, tsq-status, tsq-spec, tsq-quick, tsq-full, tsq-grill(PRD/Sub-PRD 2모드), tsq-update, tsq-delete
  - 유틸: tsq-librarian, tsq-log, tsq-prompt
- **Hook**: 8개 (Fail-closed 5 + Fail-open 3)
  - PreToolUse(Write/Edit): phase-guard → check-capability → change-scope-guard
  - PreToolUse(Bash): safe-guard
  - Stop: completion-guard → build-gate
  - PreCompact: pre-compact (상태 저장)
  - SessionStart: context-restore (컨텍스트 복원)
  - SubagentStart/Stop: 서브에이전트 라이프사이클 추적
- **Capability Token 패턴**:
...
- **테스트**: 669 테스트 (vitest), 10,018줄 TypeScript
...
| **1** | **하이브리드 Controller** (`tsq next` + hook 확장) | CLI+Hook | 중 | **#24, #25** | 미착수 |
| **1.5** | **CRITICAL Hook 3건** + **Progressive Disclosure** (병렬) | Hook+스킬 | 저~중 | 보안+무결성+토큰효율 | 미착수 |
| **2** | Hook 환경 인식 (`detect-env.sh`) + **HIGH Hook 8건** | Hook | 중 | **#25, #23** + 프로세스 강제 | 미착수 |
| **3** | **CLI 자동화 8건** (`tsq plan validate`, `tsq spec check` 등) | CLI | 중 | 상태관리+검증 | 미착수 |
